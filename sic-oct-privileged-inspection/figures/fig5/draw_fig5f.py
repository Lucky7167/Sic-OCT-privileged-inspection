...
# ================================================================ PANEL F (standalone, 数据new measured end-to-end)
fig = plt.figure(figsize=(7.8, 8.6))
axf = fig.add_subplot(111)
fig.subplots_adjust(left=0.115, right=0.965, top=0.815, bottom=0.15)

w = np.arange(5)
# measured end-to-end (microscopy + robot transfer + OCT), min/wafer:
cg_t = [12.3, 15.5, 11.7, 14.5, 14.9]   # candidate-guided OCT
ou_t = [4.1, 5.1, 4.2, 4.9, 4.8]        # selective OCT (ours)

axf.plot(w, cg_t, marker="s", ms=7, lw=1.6, color="#333333", ls="--",
         label="Candidate-guided OCT")
axf.plot(w, ou_t, marker="*", ms=15, lw=2.0, color=CSS,
         mfc=CSS, mec="#7A1F2B", label="Selective OCT (ours)")
for x, y in zip(w, cg_t):
    axf.text(x, y + 0.4, f"{y:.1f}", ha="center", fontsize=8.4, color="#333333")
for x, y in zip(w, ou_t):
    axf.text(x, y - 0.85, f"{y:.1f}", ha="center", fontsize=8.4, fontweight="bold",
             color="#B3333F")
med_cg, med_ou = float(np.median(cg_t)), float(np.median(ou_t))
axf.annotate(f"median {med_cg:.1f} \u2192 {med_ou:.1f} min (\u2212{(1-med_ou/med_cg)*100:.0f}%)",
             xy=(2, med_ou), xytext=(-0.25, 9.6), fontsize=10, fontweight="bold",
             color="#B3333F", arrowprops=dict(arrowstyle="->", color="#B3333F"))
axf.text(0.985, 0.055, "exhaustive OCT: 35.4 min/wafer (off-scale); "
         "ours = 9.8% of exhaustive\nacquisition (8.2\u201311.2% per wafer), "
         "13.6% of exhaustive end-to-end (W5)",
         transform=axf.transAxes, ha="right", va="bottom", fontsize=8.6, color="#555555")

axf.set_xticks(w); axf.set_xticklabels(WLAB, fontsize=10)
axf.set_xlim(-0.80, 4.62)
axf.set_ylim(0, 17); axf.set_ylabel("End-to-end time (min/wafer)", fontsize=11)
axf.grid(alpha=0.25); axf.set_axisbelow(True)
axf.legend(fontsize=8.8, frameon=False, loc="center right")
for s_ in ["top", "right"]:
    axf.spines[s_].set_visible(False)

fig.text(0.012, 0.985, "f", fontsize=17, fontweight="bold")
fig.text(0.5, 0.968, "Inspection time is reduced three-fold", fontsize=13, ha="center",
         fontweight="bold", va="center")
fig.savefig(OUTDIR / "Fig5f_\u5355\u72ec.png", dpi=220, facecolor="white")
plt.close(fig)
print("saved Fig5f; median:", med_cg, "->", med_ou)

# ---------------- transparent overlay (lines / stars only) ----------------
figo = plt.figure(figsize=(7.8, 8.6))
axo = figo.add_subplot(111)
figo.subplots_adjust(left=0.115, right=0.965, top=0.815, bottom=0.15)
axo.plot(w, cg_t, marker="s", ms=7, lw=1.6, color="#333333", ls="--")
axo.plot(w, ou_t, marker="*", ms=15, lw=2.0, color=CSS, mfc=CSS, mec="#7A1F2B")
axo.set_xlim(-0.80, 4.62); axo.set_ylim(0, 17); axo.axis("off")
figo.savefig(OUTDIR/"Fig5f_仅线条透明底.png", dpi=220, transparent=True)
plt.close(figo); print("saved Fig5f overlay")
