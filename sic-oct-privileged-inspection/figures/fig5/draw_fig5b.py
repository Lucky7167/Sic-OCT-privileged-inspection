...
# ================================================================ PANEL B (standalone, vertical v3 dumbbell)
# Dumbbells: open circle = top-K budget @100% recall -> filled = policy budget @100%.
# Per-wafer: thin translucent dumbbells jittered to both sides; pooled: thick opaque.
fig, axb = plt.subplots(figsize=(9.0, 8.6), dpi=220)
plt.subplots_adjust(left=0.135, right=0.98, top=0.845, bottom=0.10)

rng_b = np.random.default_rng(7)
for xv, name in enumerate(METHODS):   # entropy, margin, Anomaly, Direct, Student, Teacher
    kind, p_ = policies[name]
    mk = "o" if kind == "two-threshold referral" else "D"
    ours_ = name == "Student score"
    tk_pw = endpoint_pw(scores[name])          # per-wafer topK@100 endpoints
    pol_pw = p_["pw"]                          # per-wafer policy budgets
    tk_pool = endpoints[name]                  # pooled topK@100
    pol_pool = p_["full"][0]                   # pooled policy budget
    # ---- per-wafer dumbbells, jittered to both sides of the centre ----
    sides = rng_b.permutation([-1, -1, 1, 1, rng_b.choice([-1, 1])])
    for j in range(5):
        off = sides[j] * rng_b.uniform(0.07, 0.20)
        xj = xv + off
        axb.plot([xj, xj], [pol_pw[j], tk_pw[j]], ls="-", lw=0.8, color=COL[name],
                 alpha=0.35, zorder=2)
        axb.plot(xj, tk_pw[j], marker="o", ms=4.5, mfc="white", mec=COL[name], mew=0.9,
                 alpha=0.55, zorder=3)
        axb.plot(xj, pol_pw[j], marker=mk, ms=5, mfc=COL[name], mec="white", mew=0.5,
                 alpha=0.50, zorder=3)
    # ---- pooled dumbbell ----
    axb.plot([xv, xv], [pol_pool, tk_pool], ls="-", lw=2.6, color=COL[name],
             alpha=0.85, zorder=4, solid_capstyle="round")
    axb.plot(xv, tk_pool, marker="o", ms=10, mfc="white", mec=COL[name], mew=1.6, zorder=5)
    axb.text(xv + 0.26, tk_pool, f"{tk_pool:.1f}", va="center", ha="left",
             fontsize=9, color="#777777")
    if ours_:
        axb.plot(xv, pol_pool, marker="*", ms=22, color=CSS, mec="#7A1F2B", zorder=6)
        axb.text(xv + 0.26, pol_pool, "27.4", va="center", ha="left",
                 fontsize=11, fontweight="bold", color="#7A1F2B")
    else:
        axb.plot(xv, pol_pool, marker=mk, ms=12, mfc=COL[name], mec="#333333",
                 mew=1.2, zorder=5)
        axb.text(xv + 0.26, pol_pool, f"{pol_pool:.1f}", va="center", ha="left",
                 fontsize=11, fontweight="bold", color=COL[name])

axb.set_xticks(range(len(METHODS)))
axb.set_xticklabels(["Predictive\nentropy", "Inverse\nmargin", "Anomaly\nscore",
                     "Direct\nscore", "Student\nscore", "Teacher\nscore"], fontsize=10.5)
axb.set_xlim(-0.42, len(METHODS) - 0.60)
axb.set_ylim(0, 107)
axb.set_yticks([0, 20, 40, 60, 80, 100])
axb.set_ylabel("OCT budget @ 100% subsurface recall\n(% of candidate-guided OCT)", fontsize=11)
axb.grid(alpha=0.25); axb.set_axisbelow(True)
for s_ in ["top", "right"]:
    axb.spines[s_].set_visible(False)
axb.legend(handles=[Line2D([], [], ls="-", lw=0.9, color="#888888", alpha=0.6,
                           marker="o", ms=4, mfc="white", mec="#888888",
                           label="per-wafer dumbbells (W1\u2013W5)"),
                    Line2D([], [], marker="o", ls="", ms=9, mfc="white", mec="#555555",
                           mew=1.5, label="pooled: top-K @ 100% recall"),
                    Line2D([], [], marker="o", ls="", ms=10, mfc="#555555", mec="#333333",
                           label="pooled policy: two-threshold referral"),
                    Line2D([], [], marker="D", ls="", ms=9, mfc="#555555", mec="#333333",
                           label="pooled policy: reject option (single-threshold)"),
                    Line2D([], [], marker="*", ls="", ms=17, color=CSS, mec="#7A1F2B",
                           label="Selective OCT (ours)")],
           loc="lower right", fontsize=8.8, frameon=True, framealpha=0.92, edgecolor="#DDDDDD",
           handletextpad=0.4, labelspacing=0.5)

fig.text(0.012, 0.955, "b", fontsize=17, fontweight="bold")
fig.text(0.045, 0.955, "Referral policies cut the OCT budget far below any top-K ranking",
         fontsize=13, fontweight="bold", va="center")
axb.set_title("Top-K vs referral-policy budget @ 100% subsurface recall", fontsize=11.5,
              fontweight="bold", pad=6)
fig.text(0.5, 0.902, "dumbbells: open = top-K budget, filled = policy budget; "
         "circles = two-threshold referral, diamonds = reject option",
         ha="center", fontsize=8.6, color="#555555")
fig.text(0.5, 0.875, "thin translucent = per-wafer (W1\u2013W5, jittered), thick opaque = pooled; "
         "measured per-wafer timing", ha="center", fontsize=8.6, color="#555555")
fig.savefig(OUTDIR / "Fig5b_\u5355\u72ec.png", dpi=220, facecolor="white")
plt.close(fig)
print("saved Fig5b vertical v3 (dumbbell)")


# ================================================================ OVERLAY (transparent, no text/axes)
fig, axo = plt.subplots(figsize=(9.0, 8.6), dpi=220)
plt.subplots_adjust(left=0.135, right=0.98, top=0.845, bottom=0.10)
rng_o = np.random.default_rng(7)   # same seed -> identical jitter as the labelled figure
for xv, name in enumerate(METHODS):
    kind, p_ = policies[name]
    mk = "o" if kind == "two-threshold referral" else "D"
    ours_ = name == "Student score"
    tk_pw = endpoint_pw(scores[name]); pol_pw = p_["pw"]
    tk_pool = endpoints[name]; pol_pool = p_["full"][0]
    sides = rng_o.permutation([-1, -1, 1, 1, rng_o.choice([-1, 1])])
    for j in range(5):
        off = sides[j] * rng_o.uniform(0.07, 0.20)
        xj = xv + off
        axo.plot([xj, xj], [pol_pw[j], tk_pw[j]], ls="-", lw=0.8, color=COL[name],
                 alpha=0.35, zorder=2)
        axo.plot(xj, tk_pw[j], marker="o", ms=4.5, mfc="white", mec=COL[name], mew=0.9,
                 alpha=0.55, zorder=3)
        axo.plot(xj, pol_pw[j], marker=mk, ms=5, mfc=COL[name], mec="white", mew=0.5,
                 alpha=0.50, zorder=3)
    axo.plot([xv, xv], [pol_pool, tk_pool], ls="-", lw=2.6, color=COL[name],
             alpha=0.85, zorder=4, solid_capstyle="round")
    axo.plot(xv, tk_pool, marker="o", ms=10, mfc="white", mec=COL[name], mew=1.6, zorder=5)
    if ours_:
        axo.plot(xv, pol_pool, marker="*", ms=22, color=CSS, mec="#7A1F2B", zorder=6)
    else:
        axo.plot(xv, pol_pool, marker=mk, ms=12, mfc=COL[name], mec="#333333",
                 mew=1.2, zorder=5)
axo.set_xlim(-0.42, len(METHODS) - 0.60)
axo.set_ylim(0, 107)
axo.axis("off")
fig.savefig(OUTDIR / "Fig5b_\u4ec5\u7ebf\u6761\u900f\u660e\u5e95.png",
            dpi=220, transparent=True)
plt.close(fig)
print("saved Fig5b overlay (transparent)")
