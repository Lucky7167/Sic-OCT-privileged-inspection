...
# ================================================================ PANEL D (standalone, 数据new)
fig, axd = plt.subplots(figsize=(7.8, 8.6), dpi=220)
plt.subplots_adjust(left=0.115, right=0.965, top=0.815, bottom=0.15)

w = np.arange(5)
series = []
for n_ in METHODS:
    series.append((SHORT[n_], COL[n_], "-", endpoint_pw(scores[n_]), "o"))
for n_ in ["Anomaly score", "Direct score", "Teacher score"]:
    series.append((SHORT[n_] + " +2\u03b8", COL[n_], "--", policies[n_][1]["pw"], "s"))
for n_ in UNCERTAINTY:
    series.append((SHORT[n_] + " +reject", COL[n_], ":", policies[n_][1]["pw"], "D"))
ours_pw = [SEL_MEAS[w_] / wafer_cand[w_] * 100 for w_ in WAFERS]

for lab, c, ls_, vals, mk_ in series:
    axd.plot(w, vals, ls=ls_, lw=1.15, color=c, alpha=0.8, marker=mk_, ms=3.6, zorder=3)
axd.plot(w, ours_pw, ls="-", lw=2.4, color=CSS, marker="*",
         ms=15, mfc=CSS, mec="#7A1F2B", zorder=6)
for i_ in range(5):
    axd.text(i_, ours_pw[i_] - 4.6, f"{ours_pw[i_]:.1f}", ha="center", fontsize=8.2,
             fontweight="bold", color="#B3333F")

# one entry per method (colour is shared across top-K / +2theta / +reject styles)
handles = [Line2D([], [], ls="-", lw=1.4, color=COL[n_], marker="o", ms=4, label=SHORT[n_])
           for n_ in METHODS] + [
    Line2D([], [], ls="-", lw=2.2, color=CSS, marker="*",
           ms=12, mfc=CSS, mec="#7A1F2B", label="Selective OCT (ours)")]
axd.legend(handles=handles, loc="upper center", bbox_to_anchor=(0.5, -0.105),
           ncol=4, fontsize=7.6, frameon=False, handletextpad=0.4, columnspacing=0.7,
           labelspacing=0.35)

axd.set_xticks(w); axd.set_xticklabels(WLAB, fontsize=10)
axd.set_xlim(-0.80, 4.62); axd.set_ylim(0, 108)
axd.set_yticks([0, 20, 40, 60, 80, 100])
axd.set_ylabel("% of candidate-guided OCT", fontsize=10.5)
axd.set_title("Per-wafer budget @ 100% subsurface recall\n(solid: top-K; dashed: +two-threshold; dotted: +reject option)",
              fontsize=11, fontweight="bold", pad=6)
axd.grid(alpha=0.25); axd.set_axisbelow(True)
for s_ in ["top", "right"]:
    axd.spines[s_].set_visible(False)

fig.text(0.012, 0.968, "d", fontsize=17, fontweight="bold")
fig.text(0.045, 0.968, "Per-wafer acquisition budgets (measured timing)", fontsize=13,
         fontweight="bold", va="center")
fig.savefig(OUTDIR / "Fig5d_\u5355\u72ec.png", dpi=220, facecolor="white")
plt.close(fig)
print("saved Fig5d; ours pw =", [f"{v:.1f}" for v in ours_pw])

# ================================================================ OVERLAY (transparent, no text/axes)
fig, axo = plt.subplots(figsize=(7.8, 8.6), dpi=220)
plt.subplots_adjust(left=0.115, right=0.965, top=0.815, bottom=0.15)
for lab, c, ls_, vals, mk_ in series:
    axo.plot(w, vals, ls=ls_, lw=1.15, color=c, alpha=0.8, marker=mk_, ms=3.6, zorder=3)
axo.plot(w, ours_pw, ls="-", lw=2.4, color=CSS, marker="*", ms=15, mfc=CSS, mec="#7A1F2B",
         zorder=6)
axo.set_xlim(-0.80, 4.62); axo.set_ylim(0, 108)
axo.axis("off")
fig.savefig(OUTDIR / "Fig5d_\u4ec5\u7ebf\u6761\u900f\u660e\u5e95.png", dpi=220,
            transparent=True)
plt.close(fig)
print("saved Fig5d overlay")
