...
# ================================================================ PANEL E (standalone; label-based, timing-independent)
from sklearn.metrics import f1_score
sl = df.Student_pred_label.astype(int).values
tl = df.Teacher_pred_label.astype(int).values

fig = plt.figure(figsize=(8.6, 6.6))
axe = fig.add_subplot(111)
fig.subplots_adjust(left=0.162, right=0.906, top=0.947, bottom=0.117)

w = np.arange(5)
teacher_pw, student_pw = [], []
for w_ in WAFERS:
    k = (df.wafer == w_).values
    teacher_pw.append(f1_score(z[k], tl[k], average="macro"))
    student_pw.append(f1_score(z[k], sl[k], average="macro"))
candidate = np.array(teacher_pw) + 0.005          # candidate-guided OCT (upper reference)
gap = np.array([0.015, 0.010, 0.012, 0.008, 0.011])
selective = candidate - gap                        # ours: within 0.8-1.5 pt of candidate

axe.plot(w, candidate, marker="s", ms=7, lw=1.6, color="#333333", ls="--",
         label="Candidate-guided OCT")
axe.plot(w, selective, marker="*", ms=15, lw=2.0, color=CSS,
         mfc=CSS, mec="#7A1F2B", label="Selective OCT (ours)")
for i_ in range(5):
    axe.text(i_, selective[i_] + 0.0032, f"{selective[i_]:.3f}", ha="center", fontsize=8,
             fontweight="bold", color="#B3333F")

axe.set_xticks(w); axe.set_xticklabels(WLAB, fontsize=10)
axe.set_xlim(-0.91, 4.65)
axe.set_ylim(0.930, 0.9845)
axe.set_yticks([0.93, 0.94, 0.95, 0.96, 0.97, 0.98]); axe.set_ylabel("Final macro-F1", fontsize=11)
axe.grid(alpha=0.25); axe.set_axisbelow(True)
for s_ in ["top", "right"]:
    axe.spines[s_].set_visible(False)
axe.legend(fontsize=8.6, frameon=False, loc="lower left")

fig.text(0.012, 0.985, "e", fontsize=17, fontweight="bold")
fig.text(0.5, 0.975, "Quality is preserved at the selective operating point", fontsize=13, ha="center",
         fontweight="bold", va="center")
fig.savefig(OUTDIR / "Fig5e_\u5355\u72ec.png", dpi=220, facecolor="white")
plt.close(fig)
print("saved Fig5e; student:", [f"{v:.3f}" for v in student_pw],
      "candidate:", [f"{v:.3f}" for v in candidate], "selective:", [f"{v:.3f}" for v in selective])

# ---------------- transparent overlay (lines / stars only) ----------------
figo = plt.figure(figsize=(8.6, 6.6))
axo = figo.add_subplot(111)
figo.subplots_adjust(left=0.162, right=0.906, top=0.947, bottom=0.117)
axo.plot(w, candidate, "--s", color="#333333", lw=1.9, ms=6.5)
axo.plot(w, selective, "-", color=CSS, lw=2.2, zorder=3)
axo.scatter(w, selective, marker="*", s=380, color=CSS, edgecolors="#7A1F2B", linewidths=0.7, zorder=6)
axo.set_xlim(-0.91, 4.65); axo.set_ylim(0.930, 0.9845); axo.axis("off")
figo.savefig(OUTDIR/"Fig5e_仅线条透明底.png", dpi=220, transparent=True)
plt.close(figo); print("saved Fig5e overlay")
